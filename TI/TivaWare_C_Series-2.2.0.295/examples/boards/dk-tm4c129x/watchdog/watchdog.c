...
WatchdogIntHandler(void)
{
    ...
    //
    if(MAP_WatchdogIntStatus(WATCHDOG0_BASE, true))
    {
        //
        ...
        if(g_bFeedWatchdog0)
        {
            ...
            //
            MAP_GPIOPinWrite(LED_GREEN_GPIO_PORTBASE, LED_GREEN_GPIO_PIN,
                             (MAP_GPIOPinRead(LED_GREEN_GPIO_PORTBASE,
                                              LED_GREEN_GPIO_PIN) ^
                                              LED_GREEN_GPIO_PIN));
        }
    }
    ...
    //
    if(MAP_WatchdogIntStatus(WATCHDOG1_BASE, true))
    {
        //
        // If we have been told to stop feeding the watchdog, return immediately
        // without clearing the interrupt.  This will cause the system to reset
        ...
        if(g_bFeedWatchdog1)
        {
            ...
            //
            MAP_GPIOPinWrite(LED_AMBER_GPIO_PORTBASE, LED_AMBER_GPIO_PIN,
                             (MAP_GPIOPinRead(LED_AMBER_GPIO_PORTBASE,
                                              LED_AMBER_GPIO_PIN) ^
                                              LED_AMBER_GPIO_PIN));
        }
    }
...
main(void)
{
    ...
    MAP_SysCtlPeripheralEnable(SYSCTL_PERIPH_WDOG1);
    ...
    //
    MAP_WatchdogReloadSet(WATCHDOG0_BASE, ui32SysClock);
    MAP_WatchdogReloadSet(WATCHDOG1_BASE, 16000000);

    //
    // Enable reset generation from the watchdog timer.
    //
    MAP_WatchdogResetEnable(WATCHDOG0_BASE);
    MAP_WatchdogResetEnable(WATCHDOG1_BASE);

    //
    // Enable the watchdog timer.
    //
    MAP_WatchdogEnable(WATCHDOG0_BASE);
    MAP_WatchdogEnable(WATCHDOG1_BASE);
