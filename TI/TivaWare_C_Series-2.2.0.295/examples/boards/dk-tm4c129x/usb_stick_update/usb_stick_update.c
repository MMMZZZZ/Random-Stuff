...
ReadAppAndProgram(void)
{
    ...
    while(SimpleFsReadFileSector())
    {
        ...
        if(ui32ProgAddr == APP_START_ADDRESS)
        {
            ...
            //
            MAP_FlashProgram((uint32_t *)&g_ui8SectorBuf[8],
                             ui32ProgAddr + 8,
                             ((ui32DataSize - 8) + 3) & ~3);
        }
        ...
        else
        {
            ...
            //
            MAP_FlashProgram((uint32_t *)g_ui8SectorBuf, ui32ProgAddr,
                             (ui32DataSize + 3) & ~3);
        }
        ...
        {
            MAP_FlashProgram((uint32_t *)ui32SavedRegs, APP_START_ADDRESS,
                              8);
...
main(void)
{
    ...
    MAP_GPIODirModeSet(GPIO_PORTP_BASE, GPIO_PIN_1, GPIO_DIR_MODE_IN);
    ...
    //
    if(MAP_GPIOPinRead(GPIO_PORTP_BASE, GPIO_PIN_1) == 0)
    {
        UpdaterMain();
    }
