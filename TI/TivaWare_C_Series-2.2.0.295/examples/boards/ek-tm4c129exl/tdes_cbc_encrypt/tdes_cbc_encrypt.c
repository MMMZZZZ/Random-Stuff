...
TDESIntHandler(void)
{
    ...
    {
        MAP_DESIntClear(DES_BASE, DES_INT_DMA_CONTEXT_IN);
    ...
    {
        MAP_DESIntClear(DES_BASE, DES_INT_DMA_DATA_IN);
    ...
    {
        MAP_DESIntClear(DES_BASE, DES_INT_DMA_DATA_OUT);
                 ...
                 uint32_t ui32Length, uint32_t *pui32IV, bool bUseDMA)
{
    ...
    //
    MAP_DESConfigSet(DES_BASE, (DES_CFG_DIR_ENCRYPT | DES_CFG_TRIPLE |
                                DES_CFG_MODE_CBC));

    //
    // Write the key.
    //
    MAP_DESKeySet(DES_BASE, pui32Key);

    //
    // Write the IV.
    //
    MAP_DESIVSet(DES_BASE, pui32IV);
    ...
    if(bUseDMA)
    {
        ...
        //
        MAP_DESIntEnable(DES_BASE, (DES_INT_DMA_CONTEXT_IN |
                                    DES_INT_DMA_DATA_IN |
        ...
        //
        MAP_uDMAChannelAssign(UDMA_CH21_DES0DIN);
        MAP_uDMAChannelAttributeDisable(UDMA_CH21_DES0DIN,
                                        UDMA_ATTR_ALTSELECT |
                                        UDMA_ATTR_USEBURST |
                                        UDMA_ATTR_HIGH_PRIORITY |
                                        UDMA_ATTR_REQMASK);
        MAP_uDMAChannelControlSet(UDMA_CH21_DES0DIN | UDMA_PRI_SELECT,
                                  UDMA_SIZE_32 | UDMA_SRC_INC_32 |
                                  UDMA_DST_INC_NONE | UDMA_ARB_2 |
                                  UDMA_DST_PROT_PRIV);
        MAP_uDMAChannelTransferSet(UDMA_CH21_DES0DIN | UDMA_PRI_SELECT,
        ...
        //
        MAP_uDMAChannelAssign(UDMA_CH22_DES0DOUT);
        MAP_uDMAChannelAttributeDisable(UDMA_CH22_DES0DOUT,
                                        UDMA_ATTR_ALTSELECT |
                                        UDMA_ATTR_USEBURST |
                                        UDMA_ATTR_HIGH_PRIORITY |
                                        UDMA_ATTR_REQMASK);
        MAP_uDMAChannelControlSet(UDMA_CH22_DES0DOUT | UDMA_PRI_SELECT,
                                  UDMA_SIZE_32 | UDMA_SRC_INC_NONE |
                                  UDMA_DST_INC_32 | UDMA_ARB_2 |
                                  UDMA_SRC_PROT_PRIV);
        MAP_uDMAChannelTransferSet(UDMA_CH22_DES0DOUT | UDMA_PRI_SELECT,
                                   UDMA_MODE_BASIC,
                                   (void *)(DES_BASE + DES_O_DATA_L),
                                   (void *)pui32Dst,
                                   LengthRoundUp(ui32Length) / 4);
        ...
        // Enable DMA requests
        //
        MAP_DESDMAEnable(DES_BASE, DES_DMA_DATA_IN | DES_DMA_DATA_OUT);

        //
        // Write the length registers to start the process.
        //
        MAP_DESLengthSet(DES_BASE, ui32Length);
        ...
        // truly ready.
        // 
        MAP_uDMAChannelEnable(UDMA_CH21_DES0DIN);
        MAP_uDMAChannelEnable(UDMA_CH22_DES0DOUT);
    ...
    else
    {
        ...
        //
        MAP_DESDataProcess(DES_BASE, pui32Src, pui32Dst, ui32Length);
    }
